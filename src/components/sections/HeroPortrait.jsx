...
//       onMouseLeave={onLeave}
//       className={
//         // ✅ Dark glass chip (visible over bright photos)
//         "select-none pointer-events-auto inline-flex items-center gap-2 rounded-full " +
//         "border border-white/25 ring-1 ring-white/10 " +
//         "bg-black/45 sm:bg-black/55 " +
//         "px-3 py-1 text-xs font-semibold text-white/90 " +
//         "backdrop-blur-md " +
//         "shadow-[0_12px_28px_rgba(0,0,0,0.55)] " +
//         "transition-all duration-200 " +
//         "hover:bg-black/65 hover:border-white/35 hover:shadow-[0_18px_50px_rgba(0,0,0,0.75)] " +
//         className
//       }
//       initial={{ opacity: 0, y: 10, filter: "blur(8px)" }}
//       animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}
//       transition={{ duration: 0.45, delay, ease: "easeOut" }}
//       whileHover={finePointer ? { scale: 1.04 } : undefined}
//     >
//       <span className="h-1.5 w-1.5 rounded-full bg-white/80 shadow-[0_0_10px_rgba(255,255,255,0.45)]" />
//       {children}
//     </motion.div>
//   );
// }
...
function MagneticChip({ children, className = "", delay = 0 }) {
  ...
  return (
    <motion.div
      ...
      onMouseLeave={onLeave}
      className={
        // ✅ dark glass, always readable on photos
        "select-none pointer-events-auto inline-flex items-center gap-2 rounded-full " +
        "border border-white/25 ring-1 ring-white/10 " +
...
export default function HeroPortrait({
  ...
  // ✅ Chips layout (mobile less, desktop more)
  const chips = [
    // Mobile-visible (keep these first 4)
    {
      label: "Next.js",
      pos: "left-2 top-10",
    ...
    // Desktop-only extras (hidden on small screens)
    {
      label: "JavaScript",
      pos: "left-10 top-3",
    ...
    // {
    //   label: "jQuery",
    //   pos: "left-16 bottom-6",
    //   float: { y: [0, -8, 0], duration: 5.8 },
    //   delay: 0.68,
    //   show: "hidden lg:block",
    // },
  ];
  ...
  return (
    <div className="relative mx-auto w-full max-w-[360px] sm:max-w-[420px] lg:max-w-[520px]">
      ...
      {chips.map((c, i) => (
        ...
        >
          {/* extra readability halo behind chip */}
      ...
      <div className="relative rounded-[28px] p-[1px] bg-gradient-to-br from-foreground/25 via-transparent to-foreground/10">
        ...
        >
          ...
          >
            ...
            {/* ✅ extra vignette (premium + text readability overall) */}
