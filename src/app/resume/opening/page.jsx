...
export default function Page() {
  // ✅ Required by Next.js for useSearchParams in build/prerender
  return (
    <Suspense fallback={<ResumeOpeningFallback />}>
      <ResumeOpeningPage />
    </Suspense>
  );
}

// ✅ Fallback UI (same layout, no useSearchParams)
function ResumeOpeningFallback() {
  return (
    <ResumeOpeningUI
      status="waiting"
      secondsLeft={5}
...
function ResumeOpeningPage() {
  ...
  // ✅ Countdown + redirect (NO status toggling)
  useEffect(() => {
    if (status !== "ready") return;
    if (!payload?.fullUrl) return;
    if (redirectedRef.current) return;
...
// ✅ UI Component (NO CHANGE IN UI)
function ResumeOpeningUI({ status, secondsLeft, onCancel }) {
  return (
    <main className="fixed inset-0 z-[2147483647] min-h-[100dvh] bg-[#07070b] px-4 py-10 sm:px-6">
      {/* Background glow */}
      <div
