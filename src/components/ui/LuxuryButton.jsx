...
function makeParticles(count = 12) {
  // center se blast: random angle + distance
  return Array.from({ length: count }).map((_, i) => {
    const a = Math.random() * Math.PI * 2;
    const d = 26 + Math.random() * 22; // distance
    return {
      id: `${Date.now()}-${i}`,
      x: Math.cos(a) * d,
      y: Math.sin(a) * d,
...
export default function LuxuryButton({
  ...
  const base =
    ...
    "border backdrop-blur transition will-change-transform select-none " +
    "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-foreground/35";

  const skin = isPrimary
    ? "bg-foreground text-background border-foreground/15 shadow-[0_18px_60px_rgba(0,0,0,0.35)]"
    : "bg-foreground/[0.03] text-foreground border-border/18";
      ...
      >
        ...
        <span
          ...
          className={[
            ...
            isPrimary
              ? "bg-[radial-gradient(circle_at_30%_20%,rgba(0,0,0,0.18),transparent_60%)]"
              ...
              {burst.map((p) => (
                <motion.span
                  ...
                  className={[
                    ...
                    isPrimary ? "bg-background/70" : "bg-foreground/70",
                  ].join(" ")}
